...
| `docs/leitung-*.html` | Identitäten + Audit-Log sections unlocked; doLogin tries argus_exchange_person_code; no locked Phase-7 entries | VERIFIED | SECTIONS_MASTER includes both. secIdentitaeten/secAudit/loadIdentitaeten/loadAudit implemented. doLogin Person-code path at L683–719. lockedsec CSS rule exists (for other potential uses) but is not applied to Identitäten or Audit in the actual section dispatch or SECTIONS_MASTER array. |
